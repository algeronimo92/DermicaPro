...
describe('sendToWebhook', () => {
  ...
  ])('dado la landing %s, cuando se envía el formulario, entonces apunta a https://n8n.dermicapro.online/webhook/landing', async (_name, tratamiento, landing) => {
    // Given
    global.fetch.mockResolvedValueOnce({ ok: true });
  ...
  test('dado que el servidor responde ok, cuando se envía el webhook, entonces retorna success: true', async () => {
    ...
    // When
    const result = await sendToWebhook(formDataSinPrefijo, TRATAMIENTOS.BOTOX, LANDINGS.BOTOX);
  ...
  test('dado que el servidor responde con error 500, cuando se envía el webhook, entonces retorna success: false con código de error', async () => {
    ...
    // When
    const result = await sendToWebhook(formDataSinPrefijo, TRATAMIENTOS.HIFU, LANDINGS.HIFU);
...
describe('createWebhookPayload', () => {
  test('dado un whatsapp sin prefijo, cuando se crea el payload, entonces agrega +51 al número', () => {
    // Given / When
    const payload = createWebhookPayload(formDataSinPrefijo, TRATAMIENTOS.HIFU, LANDINGS.HIFU);
  ...
  test('dado un whatsapp que ya tiene +51, cuando se crea el payload, entonces no duplica el prefijo', () => {
    // Given / When
    const payload = createWebhookPayload(formDataConPrefijo, TRATAMIENTOS.HIFU, LANDINGS.HIFU);
  ...
  test('dado UTM params con valores vacíos o nulos, cuando se crea el payload, entonces se filtran del resultado', () => {
    ...
    // When
    const payload = createWebhookPayload(formDataSinPrefijo, TRATAMIENTOS.HIFU, LANDINGS.HIFU, utmData);
  ...
  test('dado datos del formulario completos, cuando se crea el payload, entonces incluye nombre, email y tratamiento', () => {
    // Given / When
    const payload = createWebhookPayload(formDataSinPrefijo, TRATAMIENTOS.HOLLYWOOD_PEEL, LANDINGS.HOLLYWOOD_PEEL);
