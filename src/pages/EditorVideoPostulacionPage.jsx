...
/* ──────────────────────────────────────────────────────────
   PALETTE
   #040D08  negro-verde  → fondos oscuros
   #C8A13E  gold         → CTA, acentos primarios
   #7A7A79  gris         → texto secundario
   #CF9B9B  rose         → badges, highlights suaves
   #F2F2F2  off-white    → fondos claros
────────────────────────────────────────────────────────── */
const css = `
  /* reset box */
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .dp-btn-gold:hover {
    ...
    box-shadow: 0 10px 30px rgba(200,161,62,.4);
  }
  .dp-btn-outline-gold {
  ...
  .dp-submit:hover:not(:disabled) {
    background: #b8922e;
    box-shadow: 0 8px 24px rgba(200,161,62,.4);
  ...
  .dp-input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(200,161,62,.18);
  }
  ...
  .dp-form-card {
    ...
    box-shadow: 0 20px 60px rgba(4,13,8,.12);
    padding: 2.2rem;
  }
  ...
  @media (max-width: 1023px) { .dp-form-visible .dp-sticky-cta { display: none; } }
  ...
  /* ── decorations ── */
  .dp-dots {
    background-image: radial-gradient(circle, rgba(200,161,62,.22) 1.5px, transparent 1.5px);
  ...
  .dp-badge-rose {
    background: rgba(207,155,155,.18);
    color: var(--rose);
    border: 1px solid rgba(207,155,155,.4);
  }
  ...
  .dp-tool {
    ...
    border: 1px solid rgba(200,161,62,.2);
  ...
  /* ── section divider ── */
  .dp-divider { border: none; border-top: 1px solid rgba(200,161,62,.18); }

  /* ── label ── */
  .dp-label {
    display: block; font-size: .72rem; font-weight: 700;
    letter-spacing: .08em; text-transform: uppercase;
    color: var(--gray); margin-bottom: .45rem;
  }
...
/* ════════════════════════════════════════════════════════════
   MAIN COMPONENT
════════════════════════════════════════════════════════════ */
export default function EditorVideoPostulacionPage() {
  const [formData, setFormData] = useState({
    nombre: '', apellido: '', telefono: '', email: '',
    dni: '', curriculum: null, ciudad: '', pais: '',
  });
  ...
  /* Intersection observer: hide sticky CTA when form is in view */
  useEffect(() => {
  ...
  const handleChange = (e) => {
    ...
    if (type === 'file') { val = files?.[0] || null; setDragActive(false); }
      ...
      val = value.toLowerCase().replace(/(^|\s)\S/g, c => c.toUpperCase());
  ...
  const handleDrop = (e) => {
    ...
    if (file) {
      ...
      if (errors.curriculum) setErrors(prev => ({ ...prev, curriculum: validateField('curriculum', file) }));
    }
  };
  ...
  const handleQuestionnaireSubmit = async (answers) => {
    ...
    try {
      ...
      payload.append('telefono', `+51${formData.telefono}`);
      ...
      payload.append('puesto',      NOMBRE_PUESTO);
      ...
      if (res.ok) {
        setModalType('success'); setShowModal(true); resetFlow();
        setFormData({ nombre:'', apellido:'', telefono:'', email:'', dni:'', curriculum:null, ciudad:'', pais:'' });
        setErrors({});
      } else {
        setModalType('error'); setShowModal(true); setIsSubmitting(false);
      }
    } catch {
      setModalType('error'); setShowModal(true); setIsSubmitting(false);
    }
  ...
  const inCls = (f) =>
    `dp-input ${errors[f] ? 'dp-input-err' : formData[f] ? 'dp-input-ok' : ''}`;
  ...
  return (
    <div className="antialiased" style={{ fontFamily: 'system-ui,-apple-system,sans-serif', background: 'var(--light)' }}>
      ...
      {showModal && (
        <div
          style={{ position:'fixed', inset:0, background:'rgba(4,13,8,.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:60, padding:'1rem' }}
          onClick={() => setShowModal(false)}
        >
          <div
            style={{ background:'#fff', borderRadius:'20px', maxWidth:'380px', width:'100%', padding:'2.5rem', textAlign:'center', boxShadow:'0 30px 80px rgba(0,0,0,.3)' }}
            onClick={e => e.stopPropagation()}
          >
            {modalType === 'success' ? (
              <>
                <div style={{ width:64, height:64, borderRadius:'50%', background:'rgba(200,161,62,.12)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 1.25rem', border:'2px solid rgba(200,161,62,.4)' }}>
                  <Tick />
                </div>
                <h3 style={{ fontWeight:800, fontSize:'1.4rem', color:'var(--black)', marginBottom:'.5rem' }}>¡Postulación enviada!</h3>
                <p style={{ color:'var(--gray)', fontSize:'.9rem', lineHeight:1.6, marginBottom:'1.5rem' }}>
                  Revisaremos tu perfil y te contactaremos por WhatsApp si avanzas. ¡Mucho éxito!
                </p>
                <button onClick={() => setShowModal(false)} className="dp-btn dp-btn-gold" style={{ width:'100%' }}>
                  Cerrar
                </button>
              </>
            ) : (
              <>
                ...
                <p style={{ color:'var(--gray)', fontSize:'.9rem', lineHeight:1.6, marginBottom:'1.5rem' }}>
                  No pudimos enviar tu postulación. Escríbenos directamente.
                </p>
                <a
                  ...
                  style={{ display:'block', background:'#22c55e', color:'#fff', fontWeight:700, padding:'.85rem', borderRadius:'10px', textDecoration:'none', marginBottom:'.5rem' }}
                >
                  Escribir por WhatsApp
                </a>
                <button onClick={() => setShowModal(false)} style={{ background:'none', border:'none', color:'var(--gray)', cursor:'pointer', fontSize:'.85rem', padding:'.5rem' }}>
                  Intentar de nuevo
                </button>
              </>
            )}
      ...
      {/* ════════════════════════════════════════════════════
          STICKY MOBILE CTA — shown only when form is out of view
      ════════════════════════════════════════════════════ */}
      {!formVisible && (
      ...
      <div style={{ background:'var(--black)', borderBottom:'1px solid rgba(200,161,62,.2)', padding:'.75rem 1.5rem', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <span style={{ color:'var(--white)', fontWeight:800, fontSize:'1rem', letterSpacing:'-.01em' }}>
          Dérmica<span style={{ color:'var(--gold)' }}>Pro</span>
        </span>
        <button onClick={scrollToForm} className="dp-btn dp-btn-gold" style={{ padding:'.45rem 1.2rem', fontSize:'.8rem', borderRadius:'9999px' }}>
          Postularme
        </button>
      </div>
        ...
        <div style={{ maxWidth:'1200px', margin:'0 auto', padding:'4rem 1.5rem 0' }}>
          <div className="dp-hero-grid">
            ...
            <div>
              ...
              {/* headline */}
              <h1 className="dp-hero-title dp-anim dp-anim-d2" style={{ color:'var(--white)', marginBottom:'.1em' }}>
                Editor de
              </h1>
              <h1 className="dp-hero-title dp-anim dp-anim-d2" style={{ color:'var(--gold)', marginBottom:'1rem' }}>
                Videos
              </h1>
              ...
              <p className="dp-anim dp-anim-d3" style={{ color:'rgba(255,255,255,.7)', fontSize:'1.05rem', lineHeight:1.7, marginBottom:'2rem', maxWidth:'480px' }}>
                Buscamos un editor creativo, detallista y apasionado por contar historias que conecten con nuestra audiencia y reflejen la esencia de DermicaPro.
              </p>

              {/* trust bar */}
              <div className="dp-anim dp-anim-d3" style={{ display:'flex', flexWrap:'wrap', gap:'1rem', marginBottom:'2.5rem' }}>
                {[
                  { icon: '🎬', label: 'Contenido con impacto real' },
                  { icon: '📈', label: 'Crecimiento y autonomía' },
                  { icon: '💡', label: 'Ambiente creativo' },
                ].map(b => (
              ...
              {/* tools */}
              <div className="dp-anim dp-anim-d4">
                <p style={{ color:'var(--gray)', fontSize:'.72rem', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:'.75rem' }}>
                  Dominio de:
                </p>
                <div style={{ display:'flex', flexWrap:'wrap', gap:'.6rem' }}>
                  {[
                    { abbr:'Pr', name:'Premiere Pro', bg:'#9999FF' },
                    { abbr:'FC', name:'Final Cut Pro', bg:'#E8654A' },
                    { abbr:'+', name:'Otros',         bg:'transparent', border:'1px solid var(--gold)', color:'var(--gold)' },
                  ].map(t => (
                    <div key={t.name} className="dp-tool">
                      <span style={{ width:30, height:30, borderRadius:6, background:t.bg, border:t.border||'none', color:t.color||'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontSize:'.75rem', flexShrink:0 }}>{t.abbr}</span>
                      <span style={{ color:'rgba(255,255,255,.8)', fontSize:'.82rem', fontWeight:600 }}>{t.name}</span>
                    </div>
                  ))}
                </div>
              </div>

              {/* mobile: CTA button (shown instead of sticky when near top) */}
              <div style={{ marginTop:'2rem', display:'block' }} className="lg-hidden">
                <button onClick={scrollToForm} className="dp-btn dp-btn-gold" style={{ width:'100%', fontSize:'1rem', borderRadius:'12px' }}>
            ...
            <div ref={formRef} id="postulacion-form" className="dp-anim dp-anim-d2">
              <div className="dp-form-card">
                ...
                <div style={{ marginBottom:'1.5rem', paddingBottom:'1.25rem', borderBottom:'1px solid var(--light)' }}>
                  <p style={{ color:'var(--gold)', fontSize:'.72rem', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:'.25rem' }}>
                    Paso 1 de 2 · Datos personales
                  </p>
                  <h2 style={{ color:'var(--black)', fontWeight:800, fontSize:'1.3rem', margin:0 }}>
                    Completa tu postulación
                  </h2>
                  <p style={{ color:'var(--gray)', fontSize:'.83rem', marginTop:'.3rem' }}>
                ...
                <form onSubmit={handleSubmit} noValidate>
                  <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.75rem' }}>
                      {[['nombre','Nombre','Andrés'],['apellido','Apellido','Castillo']].map(([f,l,p]) => (
                        <div key={f}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.75rem' }}>
                      <div>
                        ...
                        <div style={{ display:'flex' }}>
                          <span style={{ display:'flex', alignItems:'center', padding:'0 .75rem', border:'1.5px solid #E5E7EB', borderRight:'none', borderRadius:'10px 0 0 10px', background:'var(--light)', color:'var(--gray)', fontSize:'.85rem', whiteSpace:'nowrap' }}>
                            +51
                          </span>
                          <input type="tel" name="telefono" value={formData.telefono} onChange={handleChange}
                            placeholder="987 654 321" maxLength="9"
                            className={`dp-input ${errors.telefono ? 'dp-input-err' : formData.telefono ? 'dp-input-ok' : ''}`}
                      ...
                      <div>
                        ...
                        <input type="text" name="dni" value={formData.dni} onChange={handleChange}
                          placeholder="12345678" maxLength="8" className={inCls('dni')} />
                    ...
                    <div>
                      ...
                      <input type="email" name="email" value={formData.email} onChange={handleChange}
                        placeholder="tu@correo.com" className={inCls('email')} />
                    ...
                    <div>
                      ...
                      >
                        <input type="file" name="curriculum" accept=".pdf" onChange={handleChange}
                          className="hidden" id="cv-input" style={{ display:'none' }} />
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.75rem' }}>
                      <div>
                        <SearchableSelect
                          ...
                          value={formData.pais}
                          onChange={v => setFormData(p => ({ ...p, pais: v }))}
                      ...
                      <div style={!formData.pais ? { opacity:.5, pointerEvents:'none' } : {}}>
                        <SearchableSelect
                          ...
                          placeholder={formData.pais ? 'Busca tu ciudad...' : 'Selecciona país primero'}
                          error={errors.ciudad}
                        />
                      </div>
                    </div>
                    ...
                    <button type="submit" disabled={isSubmitting} className="dp-submit" style={{ marginTop:'.25rem' }}>
                      {isSubmitting ? (
                        <span style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'.5rem' }}>
                          <svg style={{ animation:'spin 1s linear infinite', width:18, height:18 }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
                          Procesando…
                        </span>
                      ) : (
                        <span style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'.5rem' }}>
                          Siguiente: Cuestionario <ArrowRight />
                        </span>
                      )}
                    </button>
                    ...
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:.5, marginTop:'-.25rem' }}>
                      ...
                      <span style={{ color:'var(--gray)', fontSize:'.75rem' }}>
                        Datos protegidos ·{' '}
                        <a href="/politica-privacidad" style={{ color:'var(--gray)', textDecoration:'underline' }}>Política de Privacidad</a>
                      </span>
                    </div>
      ...
      {/* ════════════════════════════════════════════════════
          SOCIAL PROOF / REQUIREMENTS — below the fold
          (for users who need convincing before filling)
      ════════════════════════════════════════════════════ */}
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:'1.5rem' }}
              ...
              <div style={{ background:'var(--white)', borderRadius:16, padding:'2rem', border:'1px solid rgba(0,0,0,.06)', boxShadow:'0 4px 20px rgba(0,0,0,.05)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'.75rem', marginBottom:'1.25rem' }}>
                  <div style={{ width:42, height:42, borderRadius:'50%', background:'var(--gold)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <svg width="20" height="20" fill="none" stroke="var(--black)" strokeWidth="2" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                    </svg>
                  </div>
                  <div>
                    <p style={{ color:'var(--gold)', fontSize:'.7rem', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase' }}>Lo que pedimos</p>
                    <p style={{ fontWeight:800, fontSize:'1.1rem', color:'var(--black)' }}>Requisitos</p>
                  </div>
                </div>
                <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:'.75rem' }}>
                  <CheckItem>Experiencia comprobable en edición de videos.</CheckItem>
                  <CheckItem>Dominio de Adobe Premiere Pro, Final Cut Pro u otros.</CheckItem>
                  <CheckItem>Conocimientos en color grading, audio y motion graphics.</CheckItem>
                  <CheckItem>Creatividad, buen ojo visual y atención al detalle.</CheckItem>
                  <CheckItem>Capacidad para trabajar bajo deadlines.</CheckItem>
                  <CheckItem>Experiencia editando reels, shorts y contenido para RRSS.</CheckItem>
                </ul>
              </div>
              ...
              <div style={{ background:'var(--black)', borderRadius:16, padding:'2rem', border:'1px solid rgba(200,161,62,.25)', boxShadow:'0 4px 20px rgba(0,0,0,.12)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'.75rem', marginBottom:'1.25rem' }}>
                  ...
                  <div>
                    <p style={{ color:'var(--gold)', fontSize:'.7rem', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase' }}>Lo que das y lo que recibes</p>
                    <p style={{ fontWeight:800, fontSize:'1.1rem', color:'var(--white)' }}>Ofrecemos</p>
                  </div>
                </div>
                <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:'.75rem' }}>
                  {[
                    'Trabajo en una marca en crecimiento del sector belleza.',
                    'Ambiente creativo y colaborativo.',
                    'Proyectos desafiantes y con propósito.',
      ...
      <section style={{ background:'var(--black)', padding:'5rem 1.5rem', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:'700px', margin:'0 auto', textAlign:'center', position:'relative' }}>
          <span className="dp-badge dp-badge-rose" style={{ marginBottom:'1.25rem', display:'inline-flex' }}>
            ¿Te interesa?
          </span>
          <h2 style={{ fontWeight:900, fontSize:'clamp(2rem,5vw,3rem)', color:'var(--white)', lineHeight:1.1, marginBottom:'.75rem' }}>
            Sé parte de nuestro<br />
            <span style={{ color:'var(--gold)' }}>equipo creativo.</span>
          </h2>
          <p style={{ color:'rgba(255,255,255,.6)', fontSize:'.95rem', lineHeight:1.7, marginBottom:'2rem' }}>
            Completa el formulario de postulación y cuéntanos tu historia. Tu próximo gran proyecto puede estar aquí.
          </p>
          <button onClick={scrollToForm} className="dp-btn dp-btn-gold" style={{ fontSize:'1rem', padding:'1rem 2.5rem', borderRadius:'9999px' }}>
            Postúlate ahora <ArrowRight size={20} />
          </button>
          <p style={{ color:'rgba(255,255,255,.35)', fontSize:'.78rem', marginTop:'1rem' }}>
      ...
      <footer style={{ background:'var(--black)', borderTop:'1px solid rgba(200,161,62,.18)', padding:'2rem 1.5rem', textAlign:'center' }}>
        ...
        <p style={{ color:'rgba(255,255,255,.25)', fontSize:'.72rem', marginTop:'.75rem' }}>
          © {new Date().getFullYear()} DermicaPro. Todos los derechos reservados.
        </p>
      </footer>
