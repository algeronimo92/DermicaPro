...
const css = `
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .dp-btn {
    ...
    font-weight: 700; border-radius: 9999px; padding: .9rem 2.2rem; font-size: .95rem;
  ...
  .dp-submit {
    ...
    font-size: 1rem; font-weight: 800; padding: 1rem; border-radius: 14px;
  ...
  .dp-input {
    ...
    border: 1.5px solid #E5E7EB; font-size: .9rem; color: #111;
  ...
  .dp-form-card {
    ...
    border: 1px solid var(--gold-mid); box-shadow: 0 20px 60px rgba(4,13,8,.12); padding: 2.2rem;
  }
  .dp-sticky-cta {
    position: fixed; bottom: 0; left: 0; right: 0; z-index: 40;
    display: none; padding: .9rem 1.25rem;
    background: var(--black); border-top: 1px solid var(--gold-mid);
  }
  ...
  .dp-tool {
    display: flex; align-items: center; gap: .6rem; padding: .55rem 1rem; border-radius: 10px;
    background: rgba(255,255,255,.05); border: 1px solid rgba(200,161,62,.2); transition: border-color .2s;
  }
...
export default function EspecialistaLaserPostulacionPage() {
  ...
  return (
    <div className="antialiased" style={{ fontFamily: 'system-ui,-apple-system,sans-serif', background: 'var(--light)' }}>
          ...
          <div style={{ background:'#fff', borderRadius:'20px', maxWidth:'380px', width:'100%', padding:'2.5rem', textAlign:'center', boxShadow:'0 30px 80px rgba(0,0,0,.3)' }} onClick={e => e.stopPropagation()}>
            {modalType === 'success' ? (
              ...
              <>
                ...
                <a href={`https://wa.me/${WHATSAPP_ERROR.replace(/\D/g,'')}?text=Hola%2C%20quiero%20postular%20a%20${encodeURIComponent(NOMBRE_PUESTO)}`} target="_blank" rel="noopener noreferrer"
        ...
        <div style={{ maxWidth:'1200px', margin:'0 auto', padding:'4rem 1.5rem 0' }}>
          <div className="dp-hero-grid">
            ...
            <div>
              ...
              <h1 className="dp-hero-title dp-anim dp-anim-d2" style={{ color:'var(--white)', marginBottom:'.1em' }}>Especialista en</h1>
              <h1 className="dp-hero-title dp-anim dp-anim-d2" style={{ color:'var(--gold)', marginBottom:'1rem' }}>Láser</h1>
              <div className="dp-anim dp-anim-d2" style={{ marginBottom:'1.5rem' }}>
                <span style={{ background:'var(--rose)', color:'var(--black)', fontWeight:700, fontSize:'.8rem', letterSpacing:'.12em', textTransform:'uppercase', padding:'.4rem 1rem', borderRadius:'4px', display:'inline-block' }}>
                  Con experiencia en estética avanzada
                </span>
              </div>
              <p className="dp-anim dp-anim-d3" style={{ color:'rgba(255,255,255,.7)', fontSize:'1.05rem', lineHeight:1.7, marginBottom:'2rem', maxWidth:'480px' }}>
                Buscamos una Esteticista o Cosmetóloga con dominio de equipos láser y habilidades comerciales. El combo perfecto para brillar en DermicaPro.
              </p>
              <div className="dp-anim dp-anim-d3" style={{ display:'flex', flexWrap:'wrap', gap:'1rem', marginBottom:'2.5rem' }}>
                {[
                  { icon: '✨', label: 'Tecnología de punta' },
                  { icon: '📈', label: 'Crecimiento profesional' },
                  { icon: '🎓', label: 'Capacitación constante' },
                ].map(b => (
              ...
              <div className="dp-anim dp-anim-d4">
                <p style={{ color:'var(--gray)', fontSize:'.72rem', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:'.75rem' }}>Equipos con los que trabajarás:</p>
                <div style={{ display:'flex', flexWrap:'wrap', gap:'.6rem' }}>
                  {[
                    { abbr:'PL', name:'Pico Láser', bg:'#6366f1' },
                    { abbr:'HF', name:'HIFU 12D',   bg:'#0ea5e9' },
                    { abbr:'IP', name:'IPL',         bg:'#f59e0b' },
                    { abbr:'+', name:'Más equipos',  bg:'transparent', border:'1px solid var(--gold)', color:'var(--gold)' },
                  ].map(t => (
              ...
              <div className="dp-form-card">
                ...
                <form onSubmit={handleSubmit} noValidate>
                  <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.75rem' }}>
                      <div>
                        ...
                        <div style={{ display:'flex' }}>
                          ...
                          <input type="tel" name="telefono" value={formData.telefono} onChange={handleChange} placeholder="987 654 321" maxLength="9"
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.75rem' }}>
                      <div>
                        <SearchableSelect label="País *" options={countriesData.countries.map(c => c.name)} value={formData.pais}
                          onChange={v => setFormData(p => ({ ...p, pais: v }))} placeholder="Busca tu país..." error={errors.pais} />
                      </div>
                      <div style={!formData.pais ? { opacity:.5, pointerEvents:'none' } : {}}>
                        <SearchableSelect label="Ciudad *" options={availableCities} value={formData.ciudad}
                          onChange={v => setFormData(p => ({ ...p, ciudad: v }))}
                          placeholder={formData.pais ? 'Busca tu ciudad...' : 'Selecciona país primero'} error={errors.ciudad} />
                      </div>
                    </div>
        ...
        <div style={{ maxWidth:'900px', margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:'1.5rem' }} className="req-grid">
            <div style={{ background:'var(--white)', borderRadius:16, padding:'2rem', border:'1px solid rgba(0,0,0,.06)', boxShadow:'0 4px 20px rgba(0,0,0,.05)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'.75rem', marginBottom:'1.25rem' }}>
                <div style={{ width:42, height:42, borderRadius:'50%', background:'var(--gold)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <svg width="20" height="20" fill="none" stroke="var(--black)" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                </div>
                <div>
              ...
              <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:'.75rem' }}>
                <CheckItem>Título o certificación en Cosmetología, Estética o carrera afín.</CheckItem>
                <CheckItem>Experiencia práctica con equipos láser u otros de medicina estética.</CheckItem>
                <CheckItem>Habilidades de venta y fidelización de pacientes.</CheckItem>
                <CheckItem>Excelente trato humano y empático en cada sesión.</CheckItem>
              </ul>
            </div>
            <div style={{ background:'var(--black)', borderRadius:16, padding:'2rem', border:'1px solid rgba(200,161,62,.25)', boxShadow:'0 4px 20px rgba(0,0,0,.12)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'.75rem', marginBottom:'1.25rem' }}>
                <div style={{ width:42, height:42, borderRadius:'50%', background:'var(--gold)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <svg width="20" height="20" fill="none" stroke="var(--black)" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                </div>
                <div>
                  <p style={{ color:'var(--gold)', fontSize:'.7rem', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase' }}>Lo que recibes</p>
                  <p style={{ fontWeight:800, fontSize:'1.1rem', color:'var(--white)' }}>Ofrecemos</p>
                </div>
              </div>
              <ul style={{ listStyle:'none', padding:0, margin:0, display:'flex', flexDirection:'column', gap:'.75rem' }}>
                {[
                  'Trabajo con equipos de última generación: Pico Láser, HIFU, IPL.',
                  'Remuneración competitiva reconociendo tu expertise.',
                  'Capacitación constante en nuevos protocolos y equipos.',
                  'Ambiente profesional y equipo comprometido.',
      ...
      <section style={{ background:'var(--black)', padding:'5rem 1.5rem', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:'700px', margin:'0 auto', textAlign:'center', position:'relative' }}>
          ...
          <h2 style={{ fontWeight:900, fontSize:'clamp(2rem,5vw,3rem)', color:'var(--white)', lineHeight:1.1, marginBottom:'.75rem' }}>
            Sé parte de nuestro<br /><span style={{ color:'var(--gold)' }}>equipo clínico.</span>
          </h2>
          <p style={{ color:'rgba(255,255,255,.6)', fontSize:'.95rem', lineHeight:1.7, marginBottom:'2rem' }}>
            Completa el formulario y cuéntanos tu experiencia. Tu próximo gran reto puede estar aquí.
          </p>
      ...
      <footer style={{ background:'var(--black)', borderTop:'1px solid rgba(200,161,62,.18)', padding:'2rem 1.5rem', textAlign:'center' }}>
        <p style={{ fontWeight:800, color:'var(--white)', marginBottom:'.3rem', letterSpacing:'-.01em' }}>Dérmica<span style={{ color:'var(--gold)' }}>Pro</span></p>
